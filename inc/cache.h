...
class CACHE : public MEMORY {
  public:
    ...
    double alpha_rates[NUM_CPUS][L1I_SET];
    uint64_t threshold_counters_kernel[NUM_CPUS][L1I_SET];
    uint64_t threshold_counters_user[NUM_CPUS][L1I_SET];
    double alpha[NUM_CPUS]; //probability of choosing original set
    uint64_t num_of_sim_miss_in_kernel_mode[NUM_CPUS],//To calculate misses in kernel mode for instructions in kernel mode in L1I
             num_of_sim_miss_in_user_mode[NUM_CPUS];//To calculate misses in user mode for instructions in user mode in L1I
    //Arohan
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
        ...
        for(int j=0;j<NUM_CPUS;j++){
            for(int i=0;i<L1I_SET;i++){
                ...
                alpha_rates[j][i]=0.5;
                threshold_counters_kernel[j][i]=1;
                threshold_counters_user[j][i]=1;
                num_of_sim_miss_in_kernel_mode[j]=0;
                num_of_sim_miss_in_user_mode[j]=0;
            }
            alpha[j]=1; 
        }
        //Arohan
    ...
    int  check_hit(PACKET *packet, uint32_t set),//Arohan added extra argument set
