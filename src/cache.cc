...
#define THRESHOLD_FOR_MISSES 0.5
//Arohan
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        //Feedback mechanism
        if(cache_type==IS_L1I &&  sim_access[fill_cpu][MSHR.entry[mshr_index].type]%THRESHOLD_ACCESSES==0){
            if(num_of_sim_miss_in_kernel_mode[fill_cpu]>num_of_sim_miss_in_user_mode[fill_cpu]){
                alpha[fill_cpu]=0.3;
            }
            else{
                alpha[fill_cpu]=0.9;
        ...
        if(cache_type==IS_L1I){
            ...
            std::mt19937 gen(rd());                          // Seed with random device
            std::bernoulli_distribution distribution(alpha[fill_cpu]);
            bool outcome = distribution(gen);


            if(outcome==true){
            //do nothing
                if(MSHR.entry[mshr_index].ip>18000000000000000000){
                    if(num_of_kernel_access_set_wise[fill_cpu][set]>THRESHOLD_ACCESSES*threshold_counters_kernel[fill_cpu][set]){
                        int minimum_set=0;
                        ...
                        //if((total-minimum)>THRESHOLD_FOR_MISSES*total){
                            ...
                            threshold_counters_kernel[fill_cpu][set]++;
                            alpha_rates[fill_cpu][set]=(double)(minimum-THRESHOLD_ACCESSES*((threshold_counters_kernel[fill_cpu][minimum_set]-1)+(threshold_counters_user[fill_cpu][minimum_set]-1)))/(double)(total-THRESHOLD_ACCESSES*((threshold_counters_kernel[fill_cpu][set]-2)+(threshold_counters_user[fill_cpu][set]-1)));
                            if(minimum==total){
                              alpha_rates[fill_cpu][set]=1;
                            }
                            //Debug Prints
                            //std::cout << "The alpha rate for set " << set << " is " << alpha_rates[fill_cpu][set] << std::endl;
                            //Debug Prints
                        //}
                        //else{
                    ...
                    if(num_of_user_access_set_wise[fill_cpu][set]>THRESHOLD_ACCESSES*threshold_counters_user[fill_cpu][set]){
                        int minimum_set=0;
                        ...
                        //if((total-minimum)>THRESHOLD_FOR_MISSES*total){
                            ...
                            threshold_counters_user[fill_cpu][set]++;
                            alpha_rates[fill_cpu][set]=(double)(minimum-THRESHOLD_ACCESSES*((threshold_counters_kernel[fill_cpu][minimum_set]-1)+(threshold_counters_user[fill_cpu][minimum_set]-1)))/(double)(total-THRESHOLD_ACCESSES*((threshold_counters_kernel[fill_cpu][set]-1)+(threshold_counters_user[fill_cpu][set]-2)));
                            if(minimum==total){
                              alpha_rates[fill_cpu][set]=1;
                            }
                            //Debug Prints
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                //Arohan (Debug Prints)
                if(cache_type == IS_L1I){
                  std::cout << " L1I WRITEBACK " << std::endl;
                } 
                //Arohan
      ...
      if (prefetch_cpu == NUM_CPUS)
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if(cache_type==IS_L1I){
                if(way<0){
                    uint32_t second_set;
                    if(PQ.entry[index].ip>18000000000000000000){
                        if(set_mappings_kernel.find(set)!=set_mappings_kernel.end()){
                            second_set=set_mappings_kernel[set];
                            num_of_kernel_access_set_wise[prefetch_cpu][set]++;
                            num_of_kernel_access_set_wise[prefetch_cpu][second_set]++;
                            if(second_set>L1I_SET){
                                cout << "ERROR MORE NO OF SETS 7" << endl;
                            }
                            way=check_hit(&PQ.entry[index],second_set);
                            if(way>=0){
                                set=second_set;
                                if(set>L1I_SET){
                                    cout << "ERROR MORE NO OF SETS 3" << endl;
                        ...
                        if(set_mappings_user.find(set)!=set_mappings_user.end()){
                            second_set=set_mappings_user[set];
                            num_of_user_access_set_wise[prefetch_cpu][set]++;
                            num_of_user_access_set_wise[prefetch_cpu][second_set]++;
                            if(second_set>L1I_SET){
                                    cout << "ERROR MORE NO OF SETS 8" << endl;
                            }
                            way=check_hit(&PQ.entry[index],second_set);
...
int CACHE::check_hit(PACKET *packet,uint32_t set) //Arohan modified arguments of the function
{
